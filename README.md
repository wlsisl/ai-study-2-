...
<h4>파이썬 기본 자료형</h4>
  ...
  ??진수를 변환하는 이유- 진수의 정의와 함께.
  ?? bit,,? 저장구조에 저장되는 값의 범위가 256가지란 소리는,, 영어 한문자가 256가지롶 표현되니까 256문자를 저장가능하단 소리?
  ??인간이 사용하는 10진수를 2진수로 변환해서 컴퓨터가 이해하도록하게함
  ...
  - 실수형(float):소수점이 있는 수를 표현하는 기본 자료형
      ...
      + 실수형에 대한 연산 
           ...
           산술연산, 관계연산( 실수형은 ==를 적용하지 않는다.) , 논리연산
 ...
 * type( b1.decode() )  #bytes변수 내용을 str로 변환한 후 자료형 확인 -> str
  ...
  - 표준 입출력
       ...
       + 표준 입력 함수:input()
            ...
            input()함수의 입력 결과는 항상 문자열(str)이다.
